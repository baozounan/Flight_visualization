...
class Engine:
    ...
    def turn_on(self, val_min, val_max):
        ...
        self.arc.create_text(0.25 * self.length, 0.8 * self.length, text=str(val_min), fill="white",
                             font=("Menlo", int(self.length / 15)))
        self.arc.create_text(0.9 * self.length, 0.55 * self.length, text=str(val_max), fill="white",
                             font=("Menlo", int(self.length / 15)))
        # self.text = self.arc.create_text(0.75 * self.length, 0.8 * self.length, text = str(self.val), fill = self.text_color)
        self.text = self.arc.create_text(0.7 * self.length, 0.75 * self.length, text="", fill=self.text_color,
                                         font=("Menlo", int(self.length / 15)))
        self.pointer = self.arc.create_line(self.center, (
        self._val2pos(0)[0] + self.center[0], self.length - (self._val2pos(0)[1] + self.center[1])), fill="green",
                                            width=3)
        self.arc.pack()

    def change_to(self, val):
        self.val = val
        self.arc.coords(self.pointer, self.center[0], self.center[1], self._val2pos(val)[0] + self.center[0],
